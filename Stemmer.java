...
class Stemmer {
	...
	private static int totalStemsCount = 0;
	private static int uniqueStems = 0;
	private static Map<String, Integer> uniqueStemsCount = new HashMap<>();
	...
	public static void stemmerCollection(int totalDocsCount) throws FileNotFoundException, UnsupportedEncodingException {
		startTime = System.currentTimeMillis();
		char[] w = new char[501];
		PrintWriter writer = new PrintWriter("stem_output.txt", "UTF-8");
		String tokensFile = "output.txt";
		...
		try {
			...
			try {
				...
				System.out.println("error reading " + tokensFile);
				// break;
			}
		} catch (FileNotFoundException e) {
			System.out.println("file " + tokensFile + " not found");
			// break;
		}
		writer.close();
		File inputFile = new File("stem_output.txt");
		Scanner input = new Scanner(inputFile);
		stemCountInformation(input, totalDocsCount);
		input.close();
	}

	private static void stemCountInformation(Scanner input, int totalDocsCount) {
		String word = "";
		int termCount = 0, index = 0;
		while (input.hasNextLine()) {
			word = input.nextLine();
			if (!uniqueStemsCount.containsKey(word))
				uniqueStemsCount.put(word, 1);
			else {
				termCount = uniqueStemsCount.get(word) + 1;
				uniqueStemsCount.put(word, termCount);
			}
			totalStemsCount++;
		}
		...
		System.out.println("*************************************************************************\n");
		System.out.println("Stemming information :: \n");
		System.out.println("Time taken :: " + totalTime + " milliseconds");
		System.out.println("Total Stems count :: " + totalStemsCount);
		System.out.println("Unique stem count :: " + uniqueStemsCount.size());
		System.out.println("Top 30 frequent stems :: " ); 
		for (Entry<String, Integer> item : sortByValue(uniqueStemsCount).entrySet()) {
			if(item.getValue() == 1)
				uniqueStems++;
			if(index++ < 30) 
				System.out.println(index + " " + item.getKey() + " " + item.getValue());
		}
		System.out.println("Stems that have unique count :: " + uniqueStems);
		System.out.println("Average number of stems in a document :: " + totalStemsCount/totalDocsCount);
	}
